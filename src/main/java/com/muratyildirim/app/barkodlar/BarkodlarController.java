...
public class BarkodlarController {
	...
	ResponseEntity<Page<BarkodlarDetayDTO>> getSearchBarkod(@RequestBody BarkodlarSearchDTO barkodu,
			Pageable pageable) {
		Page<BarkodlarDetayDTO> page = barkodlarService.searchPagedByStokKoduRBK(barkodu, pageable);
		if (page == null)
			page = barkodlarService.searchPagedByStokKodu(barkodu, pageable);
		if (page == null || page.isEmpty())
			return ResponseEntity.notFound().build();
		return ResponseEntity.ok(page);
	}
//frontend tarafinda once kurlu hesaplamalarida yap
}
